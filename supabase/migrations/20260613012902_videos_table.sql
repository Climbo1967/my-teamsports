...
create policy "Members manage videos" on public.videos for all
  using (public.is_team_coach(team_id)) with check (public.is_team_coach(team_id));
create index videos_team_idx on public.videos (team_id, created_at desc);

-- Add videos to the team site payload
